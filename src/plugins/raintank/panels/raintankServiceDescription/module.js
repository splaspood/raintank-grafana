...
function (angular, app, _, kbn) {
  ...
  var module = angular.module('grafana.panels.raintankServiceDescription', []);
  app.useModule(module);

  var converter;

  module.controller('raintankServiceDescription', function($scope, panelSrv, $modal, $q, raintankServiceType, raintankTag, raintankServiceEvent) {
    console.log($scope);
    $scope.panelMeta = {
      description : "Raintank Service details."
    };
    ...
    $scope.render = function() {
      ...
      if ($scope.panel.service) {
        $scope.service = $scope.panel.service;
    ...
    $scope.thresholds = function() {
      // Create modal (returns a promise since it may have to perform an http request)
      $scope.thresholdsModal = $modal({template: 'plugins/raintank/partials/serviceThresholdsModal.html', persist: true, show: false, backdrop: 'static', scope: $scope});
  ...
  module.controller('raintankServiceEditCtrl', function($scope, $q, $modal, raintankService, raintankServiceType, raintankLocations, raintankTag ) {
    ...
    $scope.addTag = function() {
      $scope.tagModal = $modal({template: 'plugins/raintank/partials/addTagModal.html', persist: true, show: false, backdrop: 'static', scope: $scope});
      $q.when($scope.editModal).then(function(modalEl) {
        modalEl.modal('hide');
      });
